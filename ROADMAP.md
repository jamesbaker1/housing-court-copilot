...
### 6. Live court-date sourcing (eTrack / NYSCEF) — *the marquee not-yet-built feature*
- **What:** A deterministic connector that pulls the authoritative court date + index # from eTrack/NYSCEF and sets `court_date_verified=true` via `lib/court-date.setCourtDate` — the **only** legitimate way Invariant #2 is satisfied. Today nothing can verify a court date; tenants self-confirm a model-read date (which, per the review, doesn't even persist).
...
- **Effort:** **L** (no public API — scraping or a court-data partnership; brittle, ToS-sensitive, needs monitoring). **Dependencies:** robust scraping infra (Workers + Browser Rendering or an external scrape service), index # from intake. **External account/partner:** likely a court-data access arrangement or a resilient scraping vendor; legal sign-off on ToS.
